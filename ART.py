...
class ART_3D:
    ...
    def get_sparse_Wxz(self): # Generate weight matrix for integration along y
        l, m, n = np.shape(self.S)
    ...
    def get_sparse_Wxy(self): # Generate weight matrix for integration along z
        l, m, n = np.shape(self.S)
    ...
    def run(self,target_xy,target_xz,tol=1e-3,max_iter=1000): # Run the algorithm
        t0 = time.time()
        print('Running 3D Art with Object Shape:',self.S.shape)
        p1 = target_xy.flatten() # target projection vector  1
        p2 = target_xz.flatten() # target projection vector  2
        p1 = np.reshape(p1,(p1.shape[0],1))
        p2 = np.reshape(p2,(p2.shape[0],1))
        
        self.p = np.vstack((p1,p2)) # combined target projection vector
        
        # get Weight matrices
        W1 = self.get_sparse_Wxy() # sparse matrices
        W2 = self.get_sparse_Wxz()
        self.W = sparse.vstack([W1,W2]) # combined weight matrix
        ...
        proj_xy = W1 * self.s # final xy proj.
        proj_xz = W2 * self.s # final xz proj.
        ...
        return np.reshape(proj_xy,target_xy.shape), np.reshape(proj_xz,target_xz.shape)
    
class ART_2D: # 2D ART for integration in y-direction
...
# Optimization based on Gaussian Basis functions; Also used ti generate phnatoms
